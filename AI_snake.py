...
# 等价的运动区域大小 窗体宽度/一块蛇身大小 =蛇所能移动步数（宽度上）
Cell_W = int(Window_Width / Cell_Size)
Cell_H = int(Window_Height / Cell_Size)
FIELD_SIZE = Cell_W * Cell_H  # 步数高×步数宽=最大面积
# 背景颜色
...
def draw_Grid():
    ...
    for x in range(0, Window_Width, Cell_Size):
        pygame.draw.line(Main_Display, (40, 40, 40), (x, 0), (x, Window_Height)),  # 背景,颜色,起点，终点,画一条水平线
    # 水平方向
    for y in range(0, Window_Height, Cell_Size):
        pygame.draw.line(Main_Display, (40, 40, 40), (0, y), (Window_Width, y))  # 背景,颜色,起点，终点,画一条垂直线
...
def Get_Apple_Location(snake_Coords):
    flag = True
    while flag:
        apple_location = {'x': random.randint(0, Cell_W - 1), 'y': random.randint(0, Cell_H - 1)}  # 苹果的格子必须是蛇身之外的格子
...
def Show_Start_Interface():
    ...
    title_content = title_Font.render('AI贪吃蛇', True, (255, 255, 255), (0, 0, 160))
    angle = 0
    while True:
        ...
        pressKey_content = Main_Font.render('按任意键开始游戏！按1手动游戏', True, (255, 255, 255))
    ...
    while len(queue) != 0:
        ...
        for move_direction in ['left', 'right', 'up', 'down']:
            if is_move_possible(idx, move_direction):
                if (idx + move_directions[move_direction]) == (
                        psnake[Head_index]['x'] + psnake[Head_index]['y'] * Cell_W):
                    found = True
                # 该点不是蛇身(食物是0才可以这样子写)
...
def is_tail_inside(psnake, pboard, pfood):
    ...
    for move_direction in ['left', 'right', 'up', 'down']:
        ...
        end_idx = temp_snake[-1]['x'] + temp_snake[-1]['y'] * Cell_W
        if is_move_possible(idx, move_direction) and (idx + move_directions[move_direction] == end_idx) and (
                len(temp_snake) > 3):
            result = False
    return result
...
def choose_longest_safe_move(psnake, pboard):
    ...
    for move_direction in ['left', 'right', 'up', 'down']:
        idx = psnake[Head_index]['x'] + psnake[Head_index]['y'] * Cell_W
        if is_move_possible(idx, move_direction) and (
                pboard[idx + move_directions[move_direction]] > max_distance) and (
                pboard[idx + move_directions[move_direction]] < FREE_PLACE):
    ...
    while True:
        ...
        if isAI:
            ...
            if best_move != ERR:
                ...
                if (snake_Coords[Head_index]['x'] == apple_location['x']) and (
                        snake_Coords[Head_index]['y'] == apple_location['y']):
            ...
            for event in pygame.event.get():
                if event.type == KEYDOWN:
                    if event.key == K_LEFT:
                        move_direction = 'left'
                        idx = snake_Coords[Head_index]['x'] + snake_Coords[Head_index]['y'] * Cell_W
                        if is_move_possible(idx, move_direction):
